...
export function UserListProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => refreshUsers(), [isLoggedIn, currentUser, axiosInstance, setUsers]);

  return (
    <UserListContext.Provider value={{ users, loading, refreshUsers }}>{children}</UserListContext.Provider>
  );
}
